...
export const ProductsToolbar = ({
  ...
  const hasActiveFilters =
    sortBy !== "createdAt" || sortOrder !== "desc" || categoryId !== undefined;
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <Select
        ...
        <SelectTrigger className="w-54 h-10 rounded-none border-border text-xs tracking-widest uppercase font-sans bg-transparent hover:border-primary transition-colors duration-300">
          <SelectValue placeholder="Todas as categorias" />
        </SelectTrigger>
        <SelectContent className="rounded-none">
          <SelectItem value="all" className="text-xs tracking-wide font-sans">
            Todas as categorias
          </SelectItem>
          {categories.map((cat) => (
            <SelectItem
